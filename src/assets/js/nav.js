...
document.addEventListener("DOMContentLoaded", function () {
        const hero = document.querySelector("#hero-2509");
        ...
        const backgrounds = Array.from(
            hero.querySelectorAll(".cs-background")
        );

        const infoGroups = Array.from(
            hero.querySelectorAll(".cs-info-group")
        );

        const buttons = Array.from(
            hero.querySelectorAll(".cs-button")
        );
        ...
        function showSlide(index) {
            ...
            backgrounds.forEach((background, backgroundIndex) => {
                background.classList.toggle(
                    "cs-active",
                    backgroundIndex === currentSlide
                );
            });

            infoGroups.forEach((infoGroup, infoIndex) => {
                infoGroup.classList.toggle(
                    "cs-active",
                    infoIndex === currentSlide
                );
            });
            ...
            buttons.forEach((button, buttonIndex) => {
                ...
                button.classList.toggle("cs-active", isActive);
        ...
        buttons.slice(0, slideCount).forEach((button, index) => {
